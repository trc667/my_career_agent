...
- [x] 移动端聊天页打磨（侧栏抽屉/输入栏/气泡宽度/空状态快捷提问；管理后台表格待过）
- [ ] 管理后台与聊天页骨架屏（低优先级，用户可见度低）
- [ ] 公网部署 + 固定域名（商业化前提：Docker Compose 编排 MySQL+PG+后端 → 腾讯云部署 + 正式域名 + HTTPS）—— 【用户暂定不做】
- [x] 游客受限试用（每日 3 次按 IP + GuestTrialFilter 拦截 + 聊天页游客提示条引导注册 + 分级限流兜底）
- [ ] SEO 内容落地页（665 段八股生成静态题目+答案页，长尾搜索流量获客）—— 【依赖公网部署，暂缓】
...
- [x] 学习成果分享卡（ShareCardDialog canvas 长图：周报 6 项数据 + 下载 PNG；成就/签到数据接入待扩展）
- [ ] 点踩→知识库优化闭环（踩的回答 → 后台标注 → 补知识 → 重新评估，让知识库越用越准）
- [ ] 暗色主题切换开关（EP 组件暗色变量/html class 同步/气泡/雷达图均已适配；**页面缺切换 UI 入口**，setTheme 零调用——首页顶栏加按钮）
- [ ] 其余图表 ECharts 统一（面试雷达图已换 ECharts；面试趋势折线/签到环形待换，换后 echarts 进共享 chunk 摊薄体积）
- [ ] 撞限制 VIP 引导扩展（面试配额已接 VipBenefitsDialog；简历评分/聊天积分不足场景待接）

### P3 暂缓/锦上添花 + 后端卫生（2026-08 本轮新发现）
- [ ] 游客会话孤儿数据：游客聊天写 conversation_message 但不建 conversation 行（无 userId），长期堆积——后端对匿名请求跳过消息落库或定期清理
- [ ] Controller 层 CompletableFuture.runAsync（公共 ForkJoinPool）跑长阻塞 ReAct 任务（chatReactStream），改专用线程池（Service 层同类问题已修）
- [ ] EmailCodeService.ipCounts / LoginAttemptService.attempts 只增不删的 ConcurrentHashMap（慢泄漏，量小）
- [ ] 一次请求查 4~5 次 memoryStore.getMessages（DbConversationMemoryStore 每次真实查库）
- [ ] HybridRetriever 启动 BM25 从种子文件建索引而非 DB（启动瞬间与向量库可能不一致）
- [ ] 商业化：小程序/移动端（等真实用户数据）
- [ ] PWA、邮件模板参数化、操作审计日志
- [ ] 幻觉率/端到端问答正确率评估
